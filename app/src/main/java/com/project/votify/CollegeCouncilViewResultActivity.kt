...
import com.project.votify.databinding.ActivityCollegeCouncilViewResultBinding
...
class CollegeCouncilViewResultActivity : AppCompatActivity() {

    lateinit var binding: ActivityCollegeCouncilViewResultBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityCollegeCouncilViewResultBinding.inflate(layoutInflater)
        setContentView(binding.root)

        arrayList = ArrayList()
        binding.viewDisplayResultRv.setHasFixedSize(true)
        val linearLayoutManager = LinearLayoutManager(applicationContext)
        binding.viewDisplayResultRv.layoutManager = linearLayoutManager
        cardsAdap = cardsAdapter(modelnames.clgcouncilviewresults, 5)
        cardsAdap.set(arrayList, applicationContext)
        binding.viewDisplayResultRv.adapter = cardsAdap
